...
struct lcd_param
{
    int lcdfd; //LCD设备文件描述符
    int *lcdptr; //LCD映射到内存的指针
    int lcd_w;  //LCD屏幕的分辨率
    int lcd_h; 
};

struct lcd_param mipilcd = {
    .lcdfd = -1,
    .lcdptr = NULL,
    .lcd_w = 1080,
    .lcd_h = 1920,
...
static int fb_dev_init(struct lcd_param *dev) 
{
	//打开LCD设备文件
	dev->lcdfd = open("/dev/fb0", O_RDWR);
	if (dev->lcdfd < 0) 
	{
	...
	int lret = ioctl(dev->lcdfd, FBIOGET_VSCREENINFO, &info);
	if (lret < 0) 
	{
	...
	dev->lcd_h = info.yres;
	//映射LCD到内存
	dev->lcdptr = (int *)mmap(NULL, dev->lcd_w*dev->lcd_h*4,PROT_READ | PROT_WRITE, MAP_SHARED, dev->lcdfd, 0);
	if (dev->lcdptr == NULL) 
	{
	...
	//清空LCD屏幕并填充白色背景
	memset(dev->lcdptr, 0xFF, dev->lcd_w*dev->lcd_h*4);
	return 0;
}
...
void lcd_show_rgb(struct lcd_param *dev, unsigned char *rgbdata, int w, int h)
{
	unsigned int *ptr = dev->lcdptr;
	for (int i = 0; i < h; i++) 
	{
		for (int j = 0; j < w; j++) 
		{
...
void display_on_lcd(struct v4l2_dev *dev, struct lcd_param *lcddev)
{
    struct v4l2_buffer buf;
    struct v4l2_plane planes[FMT_NUM_PLANES];
    unsigned char rgbdata[lcddev->lcd_w*lcddev->lcd_h*3];
    static int fps = 0;
    unsigned long int timestamp_start = 0;
    int second_flag = 0;
    int frame_cnt = 0;
    while(1) {
        memset(&buf, 0, sizeof(buf));
        ...
        if(dev->timestamp - timestamp_start > 1000000){
            second_flag = 1;
        }

        if(second_flag == 1){
            second_flag = 0;
            timestamp_start = dev->timestamp;
            printf("fps: %d\n", frame_cnt);
            frame_cnt = 0;
        } 
        // printf("image: sequence = %d, timestamp = %lu\n", buf.sequence, dev->timestamp);
        uyvy_to_rgb(dev->out_data, rgbdata, lcddev->lcd_w, lcddev->lcd_h);
		// yuyv_to_rgb(mptr[readbuffer.index], rgbdata, video_width, video_height);
		lcd_show_rgb(lcddev, rgbdata, lcddev->lcd_w, lcddev->lcd_h);
...
{
    struct v4l2_dev *camdev = &imx335;
    struct lcd_param *lcddev = &mipilcd;
	unsigned int fps = 0;
    unsigned int mode = 4;
    char name[40] = {0};
